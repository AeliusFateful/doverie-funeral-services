export const siteConfig = {
  name: "Доверие",
  legalName: "Ритуальная служба «Доверие»",
  // ИП — исполнитель по договору; «Доверие» используется как коммерческое обозначение.
  ip: "ИП Гусманов Сергей Равильевич",
  brand: "Доверие",
  ogrnip: "324420500052687",
  inn: "420552528499",
  title: "Ритуальные услуги в Кемерово — агентство «Доверие»",
  description:
    "Круглосуточный выезд агента за 15-30 минут. Организация похорон, оформление документов, вызов ритуального агента, кремация, памятники и ритуальные товары. Похоронное бюро полного цикла.",
  url: "https://doverie-ritual-kemerovo.ru",
  phone: "+79001068023",
  phoneDisplay: "+7 900 106-80-23",
  address: {
    street: "ул. Металлистов, 1а/3",
    city: "Кемерово",
    region: "Кемеровская область",
    country: "RU",
    postalCode: "650052",
    full: "ул. Металлистов, 1а/3, Кемерово",
  },
  geo: {
    lat: 55.401657450030015,
    lon: 86.04440689086914,
    region: "RU-KEM",
  },
  owner: {
    name: "Гусманов Сергей Равильевич",
    jobTitle: "Владелец агентства «Доверие»",
    // Raw public path; wrap with asset() in components, absUrl() in JSON-LD.
    image: "images/owner.webp",
  },
  links: {
    gis2: "https://2gis.ru/kemerovo/firm/70000001091831399",
    yandexMaps: "https://yandex.ru/maps/org/doveriye/189836077749",
  },
  rating: {
    value: 5,
    count: 99,
    countLabel: "99+ оценок",
  },
  openingHours: "Mo-Su 00:00-24:00",
  agentArrivalMinutes: "15–30",
  lastUpdated: "2026-09-01",
  services: [
    "Организация похорон",
    "Вызов ритуального агента",
    "Оформление документов",
    "Кремация",
    "Транспортировка и катафалк",
    "Прощальный зал и церемония",
    "Памятники и благоустройство",
  ],
} as const;

export function telHref(phone: string = siteConfig.phone) {
  return `tel:${phone}`;
}

export function formatLastUpdated(date: string = siteConfig.lastUpdated) {
  return `${new Date(date).toLocaleDateString("ru-RU", {
    day: "numeric",
    month: "long",
    year: "numeric",
  })}`;
}
