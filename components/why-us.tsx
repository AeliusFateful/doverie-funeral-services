import { SectionHeading } from "@/components/section-heading";
import { WhyUsSlider } from "@/components/why-us-slider";
import { whyUsReasons } from "@/lib/data/why-us";
import { icons } from "@/lib/icon-maps";

export function WhyUs() {
  return (
    <section id="why" className="section-surface relative overflow-hidden">
      <div className="section-py relative mx-auto max-w-site px-5 md:px-8">
        <SectionHeading label="Почему именно мы" title="Причины доверять нам" />

        <div className="relative mt-10 flex flex-col gap-10 lg:flex-row lg:items-stretch">
          <div className="relative h-full w-full lg:w-[46%]">
            <WhyUsSlider />
          </div>
          <div
            className="why-us-fade-overlay absolute inset-0 z-10 hidden lg:block"
            aria-hidden="true"
          />

          <div className="relative z-10 flex flex-col justify-center gap-8 sm:gap-10 lg:flex-1 lg:pl-2">
            {whyUsReasons.map((reason) => {
              const Icon = icons[reason.icon];
              return (
                <div key={reason.title} className="flex gap-4">
                  <span className="mt-1 flex h-11 w-11 shrink-0 items-center justify-center rounded-full border border-accent/30 text-accent">
                    <Icon className="h-5 w-5" aria-hidden="true" />
                  </span>
                  <div>
                    <h3 className="font-serif text-xl text-foreground">
                      {reason.title}
                    </h3>
                    <p className="mt-2 text-base leading-relaxed text-muted-foreground">
                      {reason.text}
                    </p>
                  </div>
                </div>
              );
            })}
          </div>
        </div>
      </div>
    </section>
  );
}
