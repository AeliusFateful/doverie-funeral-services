  ...
  return (
    <header className="fixed inset-x-0 pt-1 pb-1 z-50 border-b border-border/60 bg-background/70 backdrop-blur-md">
      <div className="mx-auto flex h-16 max-w-site items-center justify-between gap-2 px-2 sm:px-5 md:px-8">
        <Link href="#top" className="flex flex-col items-center gap-1" aria-label="Доверие — на главную">
          <div className="flex items-center gap-1">
            <Image className="hidden min-[480px]:block" src="/images/dove.svg" alt="" width={23} height={23} />
            <span className="font-serif text-2xl font-medium min-[480px]:text-3xl leading-none tracking-tight text-foreground mx-0.5">Доверие</span>
            <Image className="hidden min-[480px]:block transform-[scaleX(-1)]" src="/images/dove.svg" alt="" width={23} height={23} />
          </div>
        ...
        <nav className="hidden items-center gap-6 xl:gap-8 lg:flex" aria-label="Основная навигация">
          {navDesktop.map((item) => (
            <Link
              key={item.href}
              href={item.href}
        ...
        <div className="flex shrink-0 items-center gap-2 sm:gap-3">
          ...
          >
            ...
            <div className="flex min-w-0 flex-col leading-tight">
              <span className="truncate text-xs tabular-nums min-[480px]:text-sm sm:text-base">{siteConfig.phoneDisplay}</span>
      ...
      >
        <nav className="mx-auto flex max-w-site flex-col px-4 py-4 sm:px-5 md:px-8 gap-2" aria-label="Мобильная навигация">
